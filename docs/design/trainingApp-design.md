...
- **Top**: a paged graph panel (Swift Charts, MVP1-55) — swipe between "Daily load" (each day's
  ...
  `.day` one: an "Average" caption sits above the value (the mean across the displayed week, not
  one day's own reading — a `.day` subject shows no such caption at all, needing no disclosure),
  the date line shows the week's own range ("Sep 14 – Sep 20, 2026") instead of one weekday, and
  the chart highlights the whole week (`FitnessChartView`'s own week-band treatment) rather than a
  single day. "Form" always emphasizes TSB (with CTL/ATL as its usual secondary lines), matching
